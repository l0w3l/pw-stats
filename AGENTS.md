...
- Set Telegram API credentials, `PIXEL_WORLD_BOT_USERNAME`, and its matching `PIXEL_WORLD_BOT_USERNAME_ALLOWLIST` in the environment files. The sidecar session name must remain `session_data/session_name`; the named `telegram_session` volume then persists `/app/session_data/session_name.session`. Authenticate it from `docker/` with `./telegram_auth.sh` before normal startup. The sidecar is available only to FPM, queue, scheduler, and setup on an internal network.
