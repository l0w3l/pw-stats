...
- `TelegramWebAppDataClient` talks to the `access-token` FastAPI service and stores Telegram `web_app_data` permanently through the Cache facade; `PixelWorldTokenProvider` exchanges it for a permanently cached Bearer token. Only a Pixel World `403` refreshes both values and retries the rejected request once. `400` and `401` do not invalidate them.
- Minute-level `PlayersLeaderboardListData::total` samples are stored separately in `pixel_world_player_totals` for DAY/WEEK/MONTH. `pixel-world:player-totals:collect` runs every minute, uses page 1 with limit 1, and is idempotent per UTC minute. Bearer tokens use the cache store configured by `PIXEL_WORLD_TOKEN_CACHE_STORE` (use Redis in multi-process production).
...
- Set Telegram API credentials, `PIXEL_WORLD_BOT_USERNAME`, and its matching `PIXEL_WORLD_BOT_USERNAME_ALLOWLIST` in the environment files. The sidecar session name must remain `session_data/session_name`; the named `telegram_session` volume then persists `/app/session_data/session_name.session`. Authenticate it from `docker/` with `./telegram_auth.sh` before normal startup. The sidecar has no application-layer caller authentication or published host port; the private `sidecar_private` network restricts access to FPM, queue, scheduler, and setup.
