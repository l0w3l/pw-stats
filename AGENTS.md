...
- `TelegramWebAppDataClient` authenticates to the `access-token` FastAPI sidecar and caches exact Telegram `web_app_data`; `PixelWorldTokenProvider` exchanges it for a cached Bearer token. `PixelWorldLeaderboardClient` owns authenticated leaderboard HTTP, retries, and rate limiting.
...
- For a fresh Docker installation, copy both `.env.example` to `.env` and `docker/.env.example` to `docker/.env`. Docker overrides the application database host with PostgreSQL (`postgres`) and Redis host with `redis`; keep the PostgreSQL credentials in both files aligned. Generate `APP_KEY`, then generate one `ACCESS_TOKEN_INTERNAL_SECRET` (for example, `openssl rand -hex 32`) and set the same value in both files. Never use the example values in production.
- Set Telegram API credentials, `PIXEL_WORLD_BOT_USERNAME`, and its matching `PIXEL_WORLD_BOT_USERNAME_ALLOWLIST` in the environment files. The sidecar session name must remain `session_data/session_name`; the named `telegram_session` volume then persists `/app/session_data/session_name.session`. Authenticate it from `docker/` with `./telegram_auth.sh` before normal startup. The sidecar accepts only authenticated calls for an explicitly allowlisted bot and has no published host port.
