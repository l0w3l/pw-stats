...
- Set Telegram API credentials, `PIXEL_WORLD_BOT_USERNAME`, and its matching `PIXEL_WORLD_BOT_USERNAME_ALLOWLIST` in the environment files. The sidecar session name must remain `session_data/session_name`; the named `telegram_session` volume then persists `/app/session_data/session_name.session`. Authenticate it from `docker/` with `./telegram_auth.sh` before normal startup. The sidecar accepts only explicitly allowlisted bot usernames and has no published host port.
