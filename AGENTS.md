...
- Two-second `PlayersLeaderboardListData::total` samples are stored separately in `pixel_world_player_totals` for DAY/WEEK/MONTH. `pixel-world:player-totals:collect` runs every two seconds, uses page 1 with limit 1, and is idempotent per UTC two-second window. Bearer tokens use an in-process memory value plus the cache store configured by `PIXEL_WORLD_TOKEN_CACHE_STORE` (use Redis in multi-process production).
